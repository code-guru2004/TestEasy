...
export default function TakeTestPage() {
  ...
  const [stage, setStage] = useState("details"); // details, instructions, exam
  ...
  useEffect(() => {
    if (stage === "exam" && timeLeft !== null && timeLeft > 0 && !isPaused) {
  ...
  const startTest = async () => {
    try {
      setLoading(true);
      const res = await fetch(
        `https://govt-quiz-app.onrender.com/api/user/tests/${testId}/start`,
        {
          method: "POST",
          headers: {
      ...
      if (res.ok) {
        ...
        setTimeLeft(data.remainingTime || test.duration * 60);
        await fetchAttemptQuestions(data.attemptId);
        setStage("exam");
      } else {
        alert(data.msg);
        if (data.attemptId) {
          ...
          setAttemptId(data.attemptId);
          await resumeTest(data.attemptId);
        }
      }
    } catch (err) {
      console.error(err);
  ...
  const fetchAttemptQuestions = async (attemptId) => {
    try {
      const res = await fetch(
        `https://govt-quiz-app.onrender.com/api/user/attempts/${attemptId}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`
          }
      ...
      } else if (data.msg === "Time is over") {
        alert("Test time has expired!");
        router.push(`/user/test/${testId}/result`);
      }
    } catch (err) {
      console.error(err);
    }
  ...
  const resumeTest = async (attemptId) => {
    try {
      const res = await fetch(
        `https://govt-quiz-app.onrender.com/api/user/attempts/${attemptId}/resume`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`
          }
  ...
  const pauseTest = async () => {
    ...
    try {
      const res = await fetch(
        `https://govt-quiz-app.onrender.com/api/user/attempts/${attemptId}/pausetest`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const saveAnswerToDB = async (questionId, selectedOption, isMarkedForReview, timeSpent = 0) => {
    if (!attemptId) return;
    
    setSavingAnswer(true);
    try {
      const res = await fetch(
        `https://govt-quiz-app.onrender.com/api/user/attempts/${attemptId}/answer`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleAnswer = async (questionId, answer) => {
    ...
    const timeSpent = 5; // Approximate time spent on question (can be calculated)
    ...
    // Save to database
    await saveAnswerToDB(questionId, answer, markedForReview.includes(questionId), timeSpent);
  };
  ...
  const toggleMarkForReview = async (questionId) => {
    ...
    // Save to database
    const currentQ = questions[currentQuestion];
    await saveAnswerToDB(questionId, currentQ?.selectedOption || null, newMarked.includes(questionId), 0);
  };
    ...
    if (currentQuestion < questions.length - 1) {
      ...
      if (attemptId) {
        await saveAnswerToDB(
          ...
          questions[currentQuestion].selectedOption || null,
          markedForReview.includes(questions[currentQuestion]._id),
          0
        );
      }
  ...
  const goToQuestion = async (index) => {
    ...
    if (attemptId && questions[currentQuestion]) {
      await saveAnswerToDB(
        questions[currentQuestion]._id,
        questions[currentQuestion].selectedOption || null,
        markedForReview.includes(questions[currentQuestion]._id),
  ...
  const submitTest = async () => {
    ...
    try {
      const res = await fetch(
        `https://govt-quiz-app.onrender.com/api/user/attempts/${attemptId}/submit`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            ...
            <div className="p-6 space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                  ...
                  <div>
                    ...
                    <p className="font-semibold text-gray-800 dark:text-white">{test?.questions?.length || 0} questions</p>
                  </div>
                </div>
                <div className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                  ...
                  <div>
                    ...
                    <p className="font-semibold text-gray-800 dark:text-white">{test?.totalMarks} marks</p>
                  </div>
                </div>
                <div className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
              ...
              <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-4">
                ...
                <ul className="text-sm text-yellow-700 dark:text-yellow-300 space-y-1 ml-6 list-disc">
                  ...
                  <li>Each question carries {test?.questions?.[0]?.marks || 1} mark(s)</li>
                  {test?.negativeMarks > 0 && (
                    <li>Negative marking: {test.negativeMarks} marks for wrong answers</li>
                  )}
              ...
              <button
                onClick={startTest}
                disabled={loading}
                className="w-full bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 text-white font-semibold py-3 rounded-xl transition-all duration-300 transform hover:scale-[1.02] flex items-center justify-center gap-2"
              >
                {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : <Play size={18} />}
                Start Test
              </button>
            </div>
          </div>
      ...
      <div className="fixed top-0 left-0 right-0 bg-white dark:bg-gray-800 shadow-md z-40">
        <div className="flex items-center justify-between px-4 py-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-semibold text-gray-800 dark:text-white">{test?.title}</h1>
